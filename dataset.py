    ...
    def __init__(self, summary, root_dir, sequence_length, transform=None):
        ...
        for c in self.content[1:]:
            if c[0].split('/')[-1].split('-')[0] == video:
                ...
                tmp = [c]
        groups.append(tmp)
        print((groups[0][0][0:self.sequence_length], groups[0][0][self.sequence_length:]))
        #Build sequences from the same video of length sequence_length
        self.frames = [([os.path.join(self.root_dir, f[0] + 'png') for f in g[i][0:self.sequence_length]], [int(f[1]) for f in g[i][self.sequence_length:]])
